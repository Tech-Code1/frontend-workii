...
const ConfigurationSchema = z
	.object({
		headerFormat: z.union([z.string(), z.function().returns(z.string())], {
			invalid_type_error: '"headerFormat" configuration key must be a string or function(must return string)'
		}),
		bodyFormat: z.union([z.string(), z.function().returns(z.string())], {
			invalid_type_error: '"bodyFormat" configuration key must be a string or function(must return string)'
		}),
		commitTypes: z.array(
			z.object({
				value: z.string(),
		...
		allowEmptyTicketIdForBranches: z.array(z.string(), {
			invalid_type_error: '"allowEmptyTicketIdForBranches" configuration key must a an array of strings'
		}),
		...
		openAiToken: z.string({ invalid_type_error: '"openAiToken" configuration key must be a string or null' }).nullable()
	})
	.partial();
