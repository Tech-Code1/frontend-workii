...
export class WorkiiEffects {
			...
			mergeMap(({ limit, offset }) =>
				this.workiisService.getWorkiis({ limit, offset }).pipe(
					mergeMap(({ workiis, totalResults }) =>
						of({ type: WorkiiActions.listWorkiis.type, workiis, totalResults }, UiActions.stopLoading())
					),
					catchError(() =>
						of(
	...
	loadWorkii$ = createEffect(() =>
		this.actions$.pipe(
			ofType(WorkiiActions.loadWorkii),
			switchMap((action) =>
				this.workiisService.getWorkii(action.slug).pipe(
					map((workii) => {
						return WorkiiActions.loadWorkiiSucces({ workii });
					}),
					catchError(() => {
						return of(
							WorkiiActions.loadWorkiiError({ errorMessage: 'Ha ocurrido un error al intentar obtener el workii' })
						);
					})
				)
			)
	...
	loadUsersApply$ = createEffect(() =>
		this.actions$.pipe(
			ofType(WorkiiActions.loadUsersApply),
			switchMap((action) =>
				this.workiisService.getUsersApplyWorkii(action.workii, { limit: action.limit, offset: action.offset }).pipe(
					map((usersApply) => {
					...
					catchError(() => {
						return of(
							WorkiiActions.loadUsersApplyError({
								errorMessage:
									'Ha ocurrido un error al intentar obtener la lista de usuarios que han aplicado a este workii'
							})
						);
					})
				)
	...
	loadApplications$: Observable<{ type: string; applications: IApplicationUser[] } | { errorMessage: string }> =
		createEffect(() =>
			this.actions$.pipe(
				...
				exhaustMap((action) =>
					this.workiisService.getAllApplicationsWorkiiByUser(action.id, { limit: 10, offset: 0 }).pipe(
						...
						catchError(() => {
							return of(
								WorkiiActions.loadApplicationError({ errorMessage: 'Ha ocurrido un error al cargar las aplicaciones' })
							);
						})
					)
				)
			...
			switchMap((action) =>
				this.workiisService.searchWorkiis(action.searchTerm!, action.limit, action.offset).pipe(
					...
					catchError(() => {
						return of(
							UiActions.stopLoading(),
							WorkiiActions.searchWorkiiFail({ errorMessage: 'Ha ocurrido un error al cargar los workiis' })
						);
					})
				)
			)
	...
	createWorkii$: Observable<{ workii: IWorkiiCreate } | { errorMessage: string }> = createEffect(
		() =>
			this.actions$.pipe(
				...
				concatMap((action) =>
					this.workiisService.createWorkiis(action.workii).pipe(
						...
						catchError(() => {
							return of(
								WorkiiActions.errorCreateWorkii({ errorMessage: 'Ha ocurrido un error al intentar crear el Workii' })
							);
						})
					)
				)
	...
	deleteWorkii$: Observable<
		{ id: string } | { errorMessage: string } | { message: string } | { response: IApplicationResponse }
	> = createEffect(() =>
		this.actions$.pipe(
		...
		this.actions$.pipe(
			...
			mergeMap(({ result, action }) => {
				if (result.isConfirmed) {
					...
					return of(
						WorkiiActions.cancelApplicationDelete({
							message: 'Has cerrado el cuadro de confirmación sin seleccionar ninguna opción'
						})
					);
				}
	...
	removeApplication$: Observable<
		{ id: string; workii: string } | { errorMessage: string } | { message: string } | { response: IApplicationResponse }
	> = createEffect(() =>
		this.actions$.pipe(
