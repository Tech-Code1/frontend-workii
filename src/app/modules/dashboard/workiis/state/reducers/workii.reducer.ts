...
export const _workiiReducer = createReducer(
	...
	on(WorkiiActions.deleteApplicationSuccess, (state, { id, workii }) => {
		return {
			...
			workiis: state.workiis.map((worki) => {
				if (worki.id === id) {
	...
	on(WorkiiActions.applyToWorkii, (state, { user, workii }) => {
		...
		return {
			...
			workiis: state.workiis.map((workiis) => {
	...
	on(WorkiiActions.loadUsersApplySuccess, (state, { workiiUsersApplications }) => ({
		...state,
		getUsersWorkiiApply: workiiUsersApplications
	})),
	on(WorkiiActions.loadUsersApplyError, (state, { errorMessage }) => ({ ...state, errorMessage })),
	on(WorkiiActions.searchWorkii, (state) => ({ ...state })),
	on(WorkiiActions.searchWorkiiSuccess, (state, { workiis, totalSearchResults }) => ({
		...state,
		searchWorkiis: workiis,
		totalSearchResults,
		loading: false,
		notFound: false
	})),
